...
type Props = { request: null | { dc:number }, onRoll:(res:{d20:number})=>void }
...
function useAudioBeeps(){
  ...
  function beep(freq=600, dur=0.06, type: OscillatorType = 'triangle', gain=0.02){
    try{ ensure(); const ctx=ctxRef.current!, osc=ctx.createOscillator(), g=ctx.createGain();
      osc.type=type; osc.frequency.value=freq; g.gain.value=gain; osc.connect(g).connect(ctx.destination);
      osc.start(); setTimeout(()=>{try{osc.stop()}catch{}}, Math.max(1, dur*1000));
    }catch{}
  }
...
export function D20RollLauncher({ request, onRoll }: Props){
  ...
  const { seq } = useAudioBeeps()

  function drawSpin(t:number){
    const cvs = canvasRef.current; if(!cvs) return
    const ctx = cvs.getContext('2d')!
    const w = cvs.width, h = cvs.height
    ctx.clearRect(0,0,w,h); ctx.save(); ctx.translate(w/2,h/2); ctx.rotate(t/180*Math.PI)
    ctx.fillStyle='#2b2340'; ctx.strokeStyle='#5a4a7f'; ctx.lineWidth=2
    const R=44; ctx.beginPath()
    for(let i=0;i<6;i++){ const a=(i/6)*Math.PI*2; const x=Math.cos(a)*R, y=Math.sin(a)*R; if(i===0) ctx.moveTo(x,y); else ctx.lineTo(x,y) }
    ctx.closePath(); ctx.fill(); ctx.stroke(); ctx.restore()
  }
  function drawPulse(intensity:number, colorA='#661d87', colorB='#ffd166'){
    const cvs = pulseRef.current; if(!cvs) return
    const ctx=cvs.getContext('2d')!; const w=128,h=128; ctx.clearRect(0,0,w,h)
    const g = ctx.createRadialGradient(w/2,h/2,10, w/2,h/2,64)
    g.addColorStop(0, intensity>0.5?colorB:colorA); g.addColorStop(1,'transparent')
  ...
  function drawFlash(progress:number, color='#ffd166', innerR=30, outerR=64){
    ...
    g.addColorStop(0, color); g.addColorStop(.7, color+'cc'); g.addColorStop(1,'transparent')
    ctx.fillStyle=g; ctx.fillRect(0,0,w,h)
  }
  const sparks = useRef<{x:number,y:number,vx:number,vy:number,life:number}[]>([])
  function initSparks(){
    sparks.current = Array.from({length: 40}, ()=>{
      const a=Math.random()*Math.PI*2, s=2.6+Math.random()*3.8
      return {x:64,y:64,vx:Math.cos(a)*s,vy:Math.sin(a)*s,life:1}
    })
  }
  function drawSparks(){
    const cvs=sparksRef.current; if(!cvs) return
    const ctx=cvs.getContext('2d')!; ctx.clearRect(0,0,128,128); ctx.fillStyle='#ffd166'
    sparks.current.forEach(p=>{ p.x+=p.vx; p.y+=p.vy; p.vx*=0.985; p.vy*=0.985; p.life*=0.96; ctx.globalAlpha=Math.max(0,p.life); ctx.fillRect(p.x|0,p.y|0,2,2) })
    ctx.globalAlpha=1
  }
  ...
  useEffect(()=>{
    ...
    if(phase==='spin'){
      ...
      const tick=(now:number)=>{ const t=now-start; drawSpin(t/6); drawPulse(Math.sin(t/300)*.5+.5); raf=requestAnimationFrame(tick) }
  ...
  useEffect(()=>{
    if(phase==='preflash'){
      let a=0; const id=setInterval(()=>{ a+=0.2; drawFlash(a,'#661d87',24,64); if(a>=1){ clearInterval(id); setPhase('spin') } },24)
    }
  }, [phase])

  useEffect(()=>{
    if(request){
      setFadeAll(false); setShowNum(false); setGlow(false); setPhase('preflash')
      const rolled= 1 + Math.floor(Math.random()*20); setD20(rolled)
      setTimeout(()=>{
        setPhase('result')
        let a=0; const id=setInterval(()=>{ a+=0.25; drawFlash(a,'#ffd166',30,64); if(a>=1){clearInterval(id)} },24)
        if(rolled===20){ initSparks(); (function loop(){ drawSparks(); if(phase==='result') requestAnimationFrame(loop) })(); seq([880,1320,1760],120) }
        else if(rolled===1) seq([140,120,100],110)
        else seq([520,480,560],100)
        setTimeout(()=>{ setShowNum(true) }, 120)
        setTimeout(()=>{ setGlow(true) }, 680)
        setTimeout(()=>{ setFadeAll(true); setPhase('done') }, 3000)
      }, 3000)
    }
  }, [request])
  ...
  useEffect(()=>{
    if(phase==='done'){
      ...
      setTimeout(()=>{ if(d20!=null) onRoll({d20}) }, 350)
    }
  }, [phase])
  ...
  return (
    <div style={{position:'relative', width:128, height:128}} className={fadeAll?'fade-out':''}>
      ...
      {showNum && (<div className={'num '+(glow?'glow':'')} style={{fontSize:'29px', transform:'translate(0px,6px)'}}>{d20}</div>)}
    </div>
  )
}
