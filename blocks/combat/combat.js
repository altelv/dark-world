// Combat Overlay v1 — patch3 (vertical layout, sprites layer, board rotation)
(()=>{
  ...
  let gridEl = null; // created on render
  const spritesEl = qs('#sprites');
  ...
  const chatSend = qs('#sendBtn'); const chatInput = qs('#chatInput');
  const btnClose = qs('#btnCloseCombat');
  const turnL = qs('#btnTurnLeft'); const turnR = qs('#btnTurnRight');
  ...
  const state = {
    ...
    hero:  { x:0, y:Y_MAX }, // bottom-center
    enemies: [],
    obstacles: [],
    queue: [],
    draftParts: [],
  ...
  function snapshotStart(){
    state.startSnapshot = {
      hero: { ...state.hero },
      spent: { ...state.spent },
      queue: [], draftParts: [], defenseActive: false
    };
  }
  function rollbackTurn(){
    ...
    state.queue = []; state.draftParts = []; state.defenseActive = false;
    if (badgeDefense) badgeDefense.hidden = true;
    if (inputDraft) inputDraft.value = '';
    updateCounters();
    requestAnimationFrame(()=>{ positionAllSprites(); highlightHeroCell(); clearHighlights(); });
  }
  ...
  function openDevBattle(){
    ...
    state.hero = { x:0, y:Y_MAX };
    state.turn = 1;
    resetTurn();
    renderBoard();
    requestAnimationFrame(()=>{ positionAllSprites(); highlightHeroCell(); });
    showOverlay(true);
  }
  ...
  function renderBoard(){
    // grid rebuild (keep sprites layer intact)
    ...
    for(let y=Y_MIN; y<=Y_MAX; y++){
      for(let x=X_MIN; x<=X_MAX; x++){
        ...
        if (enemy){
          // no tinting, sprite will show label
        }
    ...
    // rebuild enemy sprites
    qsa('.sprite.enemy', spritesEl).forEach(n=>n.remove());
    for (const e of state.enemies){
      const s = document.createElement('div');
      s.className = 'sprite enemy';
      s.dataset.id = e.id;
      const span = document.createElement('span');
      span.textContent = e.boss ? 'Босс' : 'Враг';
      s.appendChild(span);
      spritesEl.appendChild(s);
    }
  }

  function clearHighlights(){ qsa('.cell', gridEl).forEach(c=> c.classList.remove('target','cover-highlight','hero-cell')); }
  function highlightHeroCell(){
    const c = getCell(state.hero.x, state.hero.y);
    if (c) c.classList.add('hero-cell');
  }

  function getCell(x,y){ return qs(`.cell[data-x="${x}"][data-y="${y}"]`, gridEl); }
  function addDraft(text){ state.draftParts.push(text); if (inputDraft) inputDraft.value = state.draftParts.join(' '); }

  const MOVE_VECTORS = { back:{dx:0, dy:+1}, back_diag:{dx:+1, dy:+1}, fwd:{dx:0, dy:-2}, fwd_diag:{dx:+1, dy:-1}, left:{dx:-1, dy:0}, right:{dx:+1, dy:0} };
  ...
  function queueMove(kind){
    ...
    state.queue.push({kind:'move', data:{kind}}); spend('move',1); addDraft(moveDraftText(kind));
    state.hero.x = nx; state.hero.y = ny;
    positionAllSprites(); highlightHeroCell();
  }

  function moveDraftText(kind){
    const map = { back:'Отошёл назад.', back_diag:'Отошёл назад по диагонали.', fwd:'Шагнул вперёд.', fwd_diag:'Шагнул вперёд по диагонали.', left:'Шагнул влево.', right:'Шагнул вправо.' };
    return map[kind] || 'Сместился.';
  }

  function positionAllSprites(){
    positionSprite(heroSprite, state.hero.x, state.hero.y);
    qsa('.sprite.enemy', spritesEl).forEach(s=>{
      const id = s.dataset.id;
      const e = state.enemies.find(o=>o.id===id); if (!e) return;
      positionSprite(s, e.x, e.y);
    });
  }
  function positionSprite(el, x, y){
    ...
    const tx = r.left - br.left;
    const ty = r.top - br.top;
    el.style.transform = `translate(${tx}px, ${ty}px)`;
  }

  function queueDefense(){ if (!canSpend('atk')) return toast('Нет очков атаки'); state.queue.push({kind:'defense'}); spend('atk',1); state.defenseActive = true; if (badgeDefense) badgeDefense.hidden=false; addDraft('Встал в оборону.'); }
  function queueMelee(){
    if (!canSpend('atk')) return toast('Нет очков атаки');
    const adj = targetsAdjacent(); if (adj.length===0) return toast('Нет цели рядом');
    highlightTargets(adj);
    gridEl.onclick = (e)=>{
      const t = e.target.closest('.cell.target'); if (!t) return;
      gridEl.onclick=null; clearHighlights();
      const x=+t.dataset.x, y=+t.dataset.y; const enemy=findEnemy(x,y);
      state.queue.push({kind:'melee', data:{id:enemy.id}}); spend('atk',1); addDraft(`Ударил по ${enemy.name}.`);
    };
  }
  function queueShoot(ignoreArmorPh=false,ignoreShield=false,precise=false){
    if (!canSpend('atk')) return toast('Нет очков атаки');
    const tgt = targetsLineOfSight(); if (tgt.length===0) return toast('Нет цели по линии');
    highlightTargets(tgt);
    gridEl.onclick = (e)=>{
      const t = e.target.closest('.cell.target'); if (!t) return;
      gridEl.onclick=null; clearHighlights();
      const x=+t.dataset.x, y=+t.dataset.y; const enemy=findEnemy(x,y);
      state.queue.push({kind:'shoot', data:{id:enemy.id,ignoreArmorPh,ignoreShield,precise}}); spend('atk',1);
      if (precise){ if (!canSpend('simple')) return toast('Нет ПРОСТ для Меткого выстрела'); spend('simple',1); addDraft(`Меткий выстрел по ${enemy.name}.`); }
  ...
  function queueMagic(){
    ...
    if (!canSpend('atk')) return toast('Нет очков атаки');
    const tgt = targetsMagicRange(4); if (tgt.length===0) return toast('Нет цели для магии (до 4 клеток по линии)');
    highlightTargets(tgt);
    gridEl.onclick = (e)=>{
      const t = e.target.closest('.cell.target'); if (!t) return;
      gridEl.onclick=null; clearHighlights();
      const x=+t.dataset.x, y=+t.dataset.y; const enemy=findEnemy(x,y);
      state.queue.push({kind:'magic', data:{id:enemy.id}}); spend('atk',1); addDraft(`Прочитал заклинание на ${enemy.name}.`);
    };
  }
  function queueThrow(){
    ...
    const target=candidates[0].e; state.queue.push({kind:'throw', data:{id:target.id,range}}); spend('simple',1); addDraft(`Метнул в ${target.name}.`);
  }
  ...
  function targetsAdjacent(){
    const list=[]; for (const e of state.enemies){ if (chebDist(e.x,e.y,state.hero.x,state.hero.y)===1) list.push(e); } return list;
  }
  function clearLine(x1,y1,x2,y2){
    if (x1===x2){ const[a,b]=y1<y2?[y1,y2]:[y2,y1]; for(let y=a+1;y<b;y++){ if(state.obstacles.some(o=>o.x===x1&&o.y===y)) return false; } return true; }
    if (y1===y2){ const[a,b]=x1<x2?[x1,x2]:[x2,x1]; for(let x=a+1;x<b;x++){ if(state.obstacles.some(o=>o.x===x&&o.y===y1)) return false; } return true; }
    return false;
  }
  function targetsLineOfSight(){ const list=[]; for(const e of state.enemies){ if((e.x===state.hero.x||e.y===state.hero.y)&&clearLine(state.hero.x,state.hero.y,e.x,e.y)) list.push(e);} return list; }
  function targetsMagicRange(maxDist){ const list=[]; for(const e of state.enemies){ if((e.x===state.hero.x||e.y===state.hero.y)&&clearLine(state.hero.x,state.hero.y,e.x,e.y)){ if(chebDist(state.hero.x,state.hero.y,e.x,e.y)<=maxDist) list.push(e);} } return list; }
  function highlightTargets(list){ clearHighlights(); for(const e of list){ const c=getCell(e.x,e.y); if(c) c.classList.add('target'); } }
  function onCellClick(x,y){}

  // Cover cone: only when hero adjacent, oriented from obstacle towards hero
  function highlightCoverFrom(ox,oy,enable){
    qsa('.cell',gridEl).forEach(c=>c.classList.remove('cover-highlight'));
    if(!enable) return;
    if(chebDist(ox,oy,state.hero.x,state.hero.y)>1) return;
    const vx=state.hero.x-ox, vy=state.hero.y-oy; if(vx===0&&vy===0) return;
    for(let y=Y_MIN;y<=Y_MAX;y++){
      for(let x=X_MIN;x<=X_MAX;x++){
        const dx=x-ox, dy=y-oy;
        const along=dx*vx+dy*vy; const cross=Math.abs(dx*vy-dy*vx);
        if(along>=0 && cross<=Math.abs(along)){ const c=getCell(x,y); if(c) c.classList.add('cover-highlight'); }
      }
    }
  }
  ...
  function toast(msg){
    ...
    Object.assign(t.style,{position:'fixed',left:'50%',top:'10px',transform:'translateX(-50%)',background:'rgba(0,0,0,.7)',color:'#fff',padding:'8px 12px',borderRadius:'8px',border:'1px solid var(--border)',zIndex:200});
    document.body.appendChild(t); setTimeout(()=>t.remove(),1600);
  }

  // Rotation
  function applyRotation(){ boardEl.style.transform=`rotate(${facing}deg)`; }
  if (turnL) turnL.addEventListener('click', ()=>{ facing=(facing+270)%360; applyRotation(); });
  if (turnR) turnR.addEventListener('click', ()=>{ facing=(facing+90)%360; applyRotation(); });
  ...
  window.CombatOverlay = {
    open: openDevBattle, close: ()=> overlay.classList.add('hidden'),
    ...
    loadBattle: ({hero,enemies,obstacles})=>{
      ...
      state.turn = 1; resetTurn(); renderBoard(); requestAnimationFrame(()=>{ positionAllSprites(); highlightHeroCell(); }); overlay.classList.remove('hidden');
    }
  };
})();