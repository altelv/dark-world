// No optional chaining; attach listeners explicitly.
(()=>{
  ...
  const X_MIN=-3, X_MAX=3, Y_MIN=-1, Y_MAX=4;
  ...
  function openDevBattle(){
    state.enemies = [
      ...
      { id:'e3', name:'Босс', x:0, y:4, armor_ph:2, ward_mag:1, shield:true, boss:true }
    ];
    state.obstacles = [ { x:-2, y:2 }, { x:2, y:2 }, { x:1, y:1 } ];
    state.hero = { x:0, y:0 };
    state.turn = 1;
    resetTurn();
    renderBoard();
  ...
  function makeLabel(text){
    ...
    lb.className = 'label'; lb.textContent = text;
    return lb;
  }
  ...
  function queueMove(kind){
    ...
    spend('move',1);
    const map = { back:{dx:0,dy:-1}, back_diag:{dx:1,dy:-1}, fwd:{dx:0,dy:2}, fwd_diag:{dx:1,dy:1}, left:{dx:-1,dy:0}, right:{dx:1,dy:0} };
    const v = map[kind];
  ...
  function queueDefense(){
    ...
    state.queue.push({kind:'defense'});
    spend('atk',1);
    state.defenseActive = true;
    if (badgeDefense) badgeDefense.hidden = false;
  ...
  function queueMelee(){
    ...
    boardEl.onclick = (e)=>{
      const t = e.target.closest('.cell.target');
      if (!t) return;
  ...
  function queueShoot(ignoreArmorPh=false, ignoreShield=false, precise=false){
    ...
    boardEl.onclick = (e)=>{
      ...
      state.queue.push({kind:'shoot', data});
      spend('atk',1);
      if (precise) {
        if (!canSpend('simple')) return toast('Нет ПРОСТ для Меткого выстрела');
        spend('simple',1);
        addDraft(`Меткий выстрел по ${enemy.name}.`);
      } else {
        addDraft(`Выстрелил в ${enemy.name}.`);
      }
      renderBoard();
    };
  }
  ...
  function queueMagic(){
    ...
    boardEl.onclick = (e)=>{
      ...
      spend('atk',1);
      addDraft(`Прочитал заклинание на ${enemy.name}.`);
    };
  }
  ...
  function queueThrow(){
    ...
    const candidates = state.enemies
      .map(e=> ({e,dist: chebDist(state.hero.x,state.hero.y,e.x,e.y)}))
  ...
  function endTurn(){
    ...
    const log = state.queue.map(evt=>{
      switch(evt.kind){
        ...
        case 'shoot':
          if (evt.data.precise) return `Меткий выстрел по ${ename(evt.data.id)} (игнор физ. брони${evt.data.ignoreShield?' и щита':''}).`;
          return `Выстрел по ${ename(evt.data.id)}.`;
  ...
  function targetsAdjacent(){
    ...
    for (const e of state.enemies){
      if (Math.max(Math.abs(e.x-state.hero.x), Math.abs(e.y-state.hero.y)) === 1) list.push(e);
    }
    return list;
  }
  function targetsLineOfSight(){
    const list = [];
    for (const e of state.enemies){
      if (e.x===state.hero.x || e.y===state.hero.y) {
        if (clearLine(state.hero.x,state.hero.y,e.x,e.y)) list.push(e);
      }
    }
    return list;
  }
  ...
  function clearLine(x1,y1,x2,y2){
    if (x1===x2){
      ...
      for (let y=a+1; y<b; y++){
        if (state.obstacles.some(o=>o.x===x1 && o.y===y)) return false;
      }
      return true;
    }
    if (y1===y2){
      ...
      for (let x=a+1; x<b; x++){
        if (state.obstacles.some(o=>o.x===x && o.y===y1)) return false;
      }
      return true;
    }
    return false;
  ...
  function highlightCoverFrom(ox,oy, enable){
    ...
    const dirY = Math.sign(hy - oy) || 1;
    for (let y=oy; (dirY>0? y<=Y_MAX : y>=Y_MIN); y += dirY){
      for (let x=X_MIN; x<=X_MAX; x++){
        const dx = Math.abs(x - ox);
        const dy = Math.abs(y - oy);
        if (dy>=0 && dx <= dy && ((dirY>0 && y>=oy) || (dirY<0 && y<=oy))) {
          const c = getCell(x,y);
          if (c) c.classList.add('cover-highlight');
        }
      }
    }
  }
  ...
  function toast(msg){
    ...
    Object.assign(t.style, {
      ...
      background:'rgba(0,0,0,.7)', color:'#fff', padding:'8px 12px', borderRadius:'8px',
  ...
  const turnL = qs('#btnTurnLeft'); if (turnL) turnL.addEventListener('click', ()=> { facing=(facing+270)%360; toast('Повернулся налево'); });
  const turnR = qs('#btnTurnRight'); if (turnR) turnR.addEventListener('click', ()=> { facing=(facing+90)%360; toast('Повернулся направо'); });

  qsa('.move').forEach(b=> b.addEventListener('click', ()=> queueMove(b.dataset.mv)));
  ...
  const btnPrecise = qs('#actRangerPrecise');
  function updateRangerPrecise(){
    if (btnPrecise) btnPrecise.disabled = !(character.isRanger && character.rangerPreciseShotAvailable);
  }
  if (btnPrecise) btnPrecise.addEventListener('click', ()=> {
    if (!(character.isRanger && character.rangerPreciseShotAvailable)) return toast('Недоступно');
    queueShoot(true,false,true);
  });
  updateRangerPrecise();
